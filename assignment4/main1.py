            ...
            for j in range(c):
                if(self.pridicts[i][j]):
                    ...
                    if(fenmu == 0):
                        ...
                        '''
                        if(self.pridicts[i][j] - int(self.pridicts[i][j])>=0.75):
                            self.pridicts[i][j] = int(self.pridicts[i][j]) + 1
                        elif(self.pridicts[i][j] - int(self.pridicts[i][j])>=0.25):
                            self.pridicts[i][j] = int(self.pridicts[i][j]) + 0.5
                        else:
                            self.pridicts[i][j] = int(self.pridicts[i][j])
                        '''
                    #print(self.pridicts[i][j],self.test[i][j])
                    '''
                    if(self.pridicts[i][j] > 5):
                        print("error1")
                        print(self.pridicts[i][j],self.test[i][j], i, j, self.k)        
                        self.pridicts[i][j] = 5
                    elif(self.pridicts[i][j] < 0.5):
                        print("error2")
                        print(self.pridicts[i][j],self.test[i][j], i, j, self.k)
                        self.pridicts[i][j] = 0.5
                    '''
...
data, test = readData("./dataset/ratings.csv")
